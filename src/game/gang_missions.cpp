...
bool cMissionKidnap::kidnap(sGang& gang, std::stringstream& ss, std::shared_ptr<sGirl> girl)
{
    ...
    EBaseImage girlimagetype = EBaseImage::PROFILE;
    ...
    if (!captured)
    {
        switch(AttemptCapture(gang, *girl)) {
            ...
            case EAttemptCaptureResult::CAPTURED:
                ...
                girlimagetype = EBaseImage::DEATH;
                dungeonreason = DUNGEON_GIRLKIDNAPPED;
        ...
        if (!captured_girls.empty())
        {
            ...
            for (auto& girl: captured_girls)
            {
                if (girl.unique())
                {
                    ...
                    girl->AddMessage(NGmsg.str(), EBaseImage::PROFILE, EVENT_GANG);
        ...
        if (g_Dice.percent(beasts * 5))
        {
            ...
            if (item)
            {
                ...
                if (girl && girl->add_item(item))                        // see if a girl can take it
                {
                    ...
                    girl->AddMessage(gss.str(), EBaseImage::PROFILE, EVENT_GANG);
                    beasts--;
                    ss << "\n" << gss.str() << "\n";
                }
