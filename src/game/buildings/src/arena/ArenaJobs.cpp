...
namespace deprecated {
    ...
    CityGuard::CityGuard() : cSimpleJob(JOB_CITYGUARD, "ArenaCityGuard.xml",
                                        {EActivity::FIGHTING, 10, EImageBaseType::SECURITY, true}) {
    ...
    FightBeasts::FightBeasts() : cSimpleJob(JOB_FIGHTBEASTS, "ArenaFightBeasts.xml",
                                            {EActivity::FIGHTING, 100, EImageBaseType::COMBAT, true}) {
    ...
    FightGirls::FightGirls() : cSimpleJob(JOB_FIGHTARENAGIRLS, "ArenaFightGirls.xml",
                                          {EActivity::FIGHTING, 50, EImageBaseType::COMBAT, true}) {
...
#pragma endregion
    ...
    FightTraining::FightTraining() : cSimpleJob(JOB_FIGHTTRAIN, "ArenaFightTrain.xml",
                                                {EActivity::FIGHTING, 20, EImageBaseType::COMBAT, true}) {
    }
    ...
    bool FightTraining::JobProcessing(sGirl& girl, cBuilding& brothel, bool is_night) {
        ...
        if (train == 1) {
            sgCmb = skill;
            ss << "She trains in general combat techniques.\n";
        }
        else sgCmb = uniform(0, 1);
        if (train == 2) {
            sgMag = skill;
            ss << "She trains to control her magic.\n";
        }
        else sgMag = uniform(0, 1);
        if (train == 3) {
            sgAgi = skill;
            ss << "She trains her agility.\n";
        }
        else sgAgi = uniform(0, 1);
        if (train == 4) {
            sgCns = skill;
            ss << "She works on her constitution.\n";
        }
        else sgCns = uniform(0, 1);
        if (train == 5) {
            sgStr = skill;
            ss << "She lifts weights to get stronger.\n";
        }
...
void RegisterArenaJobs(cJobManager& mgr) {
    mgr.register_job(wrap(std::make_unique<deprecated::CityGuard>()));
    mgr.register_job(wrap(std::make_unique<deprecated::FightBeasts>()));
    mgr.register_job(wrap(std::make_unique<deprecated::FightGirls>()));
    mgr.register_job(wrap(std::make_unique<deprecated::FightTraining>()));
