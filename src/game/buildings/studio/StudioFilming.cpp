...
sWorkJobResult cFilmSceneJob::DoWork(sGirl& girl, bool is_night) {
    ...
    if (m_IsForced) {
        if(girl.has_active_trait("Iron Will")) {
            ...
            {
                girl.AddMessage("${name}'s unwilling degradation has shattered her iron will.", EBaseImage::TORTURE, EVENT_GOODNEWS);
                return {true, 0, 0, 0};
            }
        } else {
            if (girl.gain_trait("Mind Fucked", 2))
            {
                girl.AddMessage("${name} has become Mind Fucked from the forced degradation.", EBaseImage::TORTURE, EVENT_WARNING);
                return {true, 0, 0, 0};
            }
        }
    }
...
void cFilmSceneJob::update_enjoyment(sGirl& girl) const {
    ...
    if (m_Enjoyment > old_enjoyment + 2) {
        ...
        if(chance(25 + 5 * delta)) {
            ...
            girl.AddMessage(enjoy_message.str(), EBaseImage::PROFILE, EVENT_GOODNEWS);
        }
        ...
        if(chance(25 + 5 * delta)) {
            ...
            girl.AddMessage(enjoy_message.str(), EBaseImage::PROFILE, EVENT_WARNING);
        }
    }
}
...
cFilmSceneJob::cFilmSceneJob(JOBS job, const char* xml, EBaseImage event_image, SceneType scene, SexAction sex) :
