...
bool DoctorJob::JobProcessing(sGirl& girl, IBuilding& brothel, bool is_night) {
    ...
    girl.AddMessage(ss.str(), EBaseImage::NURSE, is_night ? EVENT_NIGHTSHIFT : EVENT_DAYSHIFT);
...
IGenericJob::eCheckWorkResult DoctorJob::CheckWork(sGirl& girl, bool is_night) {
    ...
    if (girl.is_slave())
    {
        ...
        girl.AddMessage(ss.str(), EBaseImage::NURSE, EVENT_WARNING);
...
bool NurseJob::JobProcessing(sGirl& girl, IBuilding& brothel, bool is_night) {
    ...
    sImageSpec image_spec = girl.MakeImageSpec(EBaseImage::NURSE);
    ...
    if (sex)
    {
        ...
        {
            image_spec.BasicImage = EBaseImage::ANAL;
            girl.anal(2);
        }
    ...
    else if (hand)
    {
        ...
        girl.handjob(2);
        image_spec.BasicImage = EBaseImage::HAND;
    }
    else if (les)
    {
        brothel.m_Happiness += uniform(30, 100);
        image_spec.BasicImage = EBaseImage::LESBIAN;
        girl.lesbian(2);
    }
...
IGenericJob::eCheckWorkResult InternJob::CheckWork(sGirl& girl, bool is_night) {
    ...
    if (girl.medicine() + girl.intelligence() + girl.charisma() >= 300)
    {
        ...
        girl.AddMessage(ss.str(), EBaseImage::NURSE, EVENT_GOODNEWS);
        return IGenericJob::eCheckWorkResult::IMPOSSIBLE;
    }
