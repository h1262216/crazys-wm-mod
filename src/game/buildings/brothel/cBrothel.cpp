    ...
    */
    m_Girls->apply([&](sGirl& current){
        ...
        */
        ...
        if (!MatronMsg.empty())
        {
            current.AddMessage(MatronMsg, EBaseImage::PROFILE, is_night ? EVENT_NIGHTSHIFT : EVENT_DAYSHIFT);
            MatronMsg = "";
        }

        if (!MatronWarningMsg.empty())
        {
            current.AddMessage(MatronWarningMsg, EBaseImage::PROFILE, EVENT_WARNING);
            MatronWarningMsg = "";
        }
        /*
        *        MATRON CODE END
...
/*
    ...
    */
    if (girl.disobey_check(ACTION_GENERAL))    // check if the girl will run away
    {
        ...
        girl.AddMessage("She ran away.", EBaseImage::PROFILE, EVENT_DANGER);
...
void do_food_and_digs(IBuilding& brothel, sGirl& girl)
{
    ...
    */
    ...
    if (ss.str().length() > 0)    girl.AddMessage(ss.str(), EBaseImage::PROFILE, EVENT_GOODNEWS);
}
