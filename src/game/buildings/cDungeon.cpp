    ...
    for(auto& current : m_Girls)
    {
        ...
        */
        ...
        if (girl->health() <= 0)
        {
            ...
            msg.str(""); ss.str("");
            girl->AddMessage("${name} has died in the dungeon.", EBaseImage::DEATH, EVENT_DANGER);
            girl->AddMessage("${name} has died.  Her body will be removed by the end of the week.\n", EBaseImage::DEATH, EVENT_SUMMARY);
            ...
            if (m_TortureDone)
            {
                ...
                TorturerGirlref->AddMessage(msg.str(), EBaseImage::DOM, EVENT_DUNGEON);
            }
        ...
        */
        ...
        EBaseImage imgtype = EBaseImage::JAIL;
