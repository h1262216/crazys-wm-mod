...
bool HouseCook::JobProcessing(sGirl& girl, IBuilding& brothel, bool is_night) {
    ...
    // do all the output
    girl.AddMessage(ss.str(), EBaseImage::COOK, is_night ? EVENT_NIGHTSHIFT : EVENT_DAYSHIFT);
...
#pragma region //    Job setup                //
    ...
    sImageSpec image_spec = girl.MakeImageSpec(EBaseImage::PUPPYGIRL);
            ...
            }else if (roll_b >= 90 && is_virgin(girl))
            {
                if (roll_c >= 85)
                {
                    if (dispmod >= 0)/*NICE DISP*/
                    {
                        ...
                        image_spec.BasicImage = EBaseImage::SEX;
                    }
                    else
                    {
                        ...
                        ob += 4;
                        girl.lose_trait("Virgin");
                        image_spec.BasicImage = EBaseImage::BEAST;
                    }
                }
                else
                {
                    ...
                    training += 4;
                    ob += 4;
                    image_spec.BasicImage = EBaseImage::ORAL;
                }
            }
...
sWorkJobResult PersonalTraining::DoWork(sGirl& girl, bool is_night) {
    ...
    if (roll_b <= 10 && girl.strip() < 100)
    {
        ...
        girl.AddMessage(ss.str(), EBaseImage::STRIP, is_night ? EVENT_NIGHTSHIFT : EVENT_DAYSHIFT);
    }
    ...
    else if (roll_b <= 27 && girl.tittysex() < 100 && brothel->is_sex_type_allowed(SKILL_TITTYSEX))
    {
        ...
        girl.AddMessage(ss.str(), EBaseImage::TITTY, is_night ? EVENT_NIGHTSHIFT : EVENT_DAYSHIFT);
    }
    else if (roll_b <= 34 && girl.handjob() < 100 && brothel->is_sex_type_allowed(SKILL_HANDJOB))
    {
        ...
        girl.AddMessage(ss.str(), EBaseImage::HAND, is_night ? EVENT_NIGHTSHIFT : EVENT_DAYSHIFT);
    }
    else if (roll_b <= 40 && girl.footjob() < 100 && brothel->is_sex_type_allowed(SKILL_FOOTJOB))
    {
        ...
        girl.AddMessage(ss.str(), EBaseImage::FOOT, is_night ? EVENT_NIGHTSHIFT : EVENT_DAYSHIFT);
    }
    else if (roll_b <= 50 && girl.oralsex() < 100 && brothel->is_sex_type_allowed(SKILL_ORALSEX))
    {
        ...
        girl.AddMessage(ss.str(), EBaseImage::ORAL, is_night ? EVENT_NIGHTSHIFT : EVENT_DAYSHIFT);
    }
    else if (roll_b <= 70 && girl.normalsex() < 100 && brothel->is_sex_type_allowed(SKILL_NORMALSEX))
    {
        ...
        girl.AddMessage(ss.str(), EBaseImage::SEX, is_night ? EVENT_NIGHTSHIFT : EVENT_DAYSHIFT);
    ...
    else if (roll_b <= 80 && girl.anal() < 100 && brothel->is_sex_type_allowed(SKILL_ANAL))
    {
        ...
        girl.AddMessage(ss.str(), EBaseImage::ANAL, is_night ? EVENT_NIGHTSHIFT : EVENT_DAYSHIFT);
    }
    ...
    else if (roll_b <= 100 && girl.bdsm() < 100 && brothel->is_sex_type_allowed(SKILL_BDSM))
    {
        ...
        girl.AddMessage(ss.str(), EBaseImage::BDSM, is_night ? EVENT_NIGHTSHIFT : EVENT_DAYSHIFT);
        girl.calc_pregnancy(&g_Game->player(), 1.0);
    }
    ...
    else
    {
        ...
        girl.AddMessage(ss.str(), EBaseImage::ECCHI, is_night ? EVENT_NIGHTSHIFT : EVENT_DAYSHIFT);
    }
