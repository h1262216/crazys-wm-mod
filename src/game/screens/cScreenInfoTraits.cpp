...
cScreenInfoTraits::cScreenInfoTraits() = default;

void cScreenInfoTraits::setup_callbacks() {
    SetListBoxSelectionCallback(m_TraitsList_id, [&](int id){
        load_effects(id);
    });
...
void cScreenInfoTraits::init(bool back) {
    ...
    for(auto& trait: m_SortedTraits) {
        ...
        AddToListBox(m_TraitsList_id, counter, g_Game->traits().lookup(trait.c_str())->display_name(), color);
        ++counter;
    }
}
...
void cScreenInfoTraits::load_effects(int trait_id) {
    ...
    EditTextItem(trait_data->desc(), m_Description_id);
...
std::shared_ptr<cInterfaceWindow> screens::cInfoTraitsScreenBase::create() {
    return std::make_shared<cScreenInfoTraits>();
}
