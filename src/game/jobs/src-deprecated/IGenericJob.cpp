...
namespace deprecated {
    ...
    public:
        cJobWrapper(JOBS j, std::function<sWorkJobResult(sGirl&, bool, cRng&)> w,
                    std::function<double(const sGirl&, bool)> p,
        ...
        cJobWrapper& full_time() {
            m_Info.FullTime = true;
            return *this;
        };

        cJobWrapper& free_only() {
...
#define REGISTER_JOB_MANUAL(J, Wf, Pf, Brief, Desc)                                     \
    [&]() -> auto& {                                                                    \
    auto new_job = std::make_unique<deprecated::cJobWrapper>(J, Work##Wf, JP_##Pf, Brief, Desc);    \
    auto ptr = new_job.get();                                                           \
    mgr.register_job(std::make_unique<JobWrapper>(std::move(new_job)));                 \
    return *ptr;                                                                        \
    }()
