...
sWorkJobResult BrothelMatronJob::DoWork(sGirl& girl, bool is_night) {
        ...
        !chance(girl.pcfear()))                    // Fear may keep her from stealing
    {
        ...
        if (roll_b < brothel->m_SecurityLevel)
        {
            ...
            ss << "\n" << warning.str() << "\n";
            girl.AddMessage(warning.str(), EBaseImage::PROFILE, EVENT_WARNING);
        }
        else
        {
    ...
    if (is_addict(girl, true) && chance(girl.m_Withdrawals * 20))
    {
        ...
        std::stringstream warning;
        EBaseImage warningimage = EBaseImage::PROFILE;
        switch (method)
        {
            case 1:
