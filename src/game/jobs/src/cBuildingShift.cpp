...
void cBuildingShift::begin_shift(bool is_night) {
    ...
    apply_to_girls([&](const sGirl& girl){
        g_Game->job_manager().handle_pre_shift(get_girl_data(girl));
    });
};
...
void cBuildingShift::end_shift() {
    apply_to_girls([&](const sGirl& current)
    {
...
void cBuildingShift::update_girls(EJobPhase phase) {
    apply_to_girls([&](const auto& current) {
        auto& shift = get_girl_data(current);
...
std::unique_ptr<IBuildingShift> IBuildingShift::create(cBuilding* building) {
    return std::make_unique<cBuildingShift>(building);
}
