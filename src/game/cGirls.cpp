...
void cGirls::LevelUp(sGirl& girl)
{
    ...
    girl.AddMessage(ss.str(), EBaseImage::PROFILE, EVENT_LEVELUP);
    ss.str("");
    ...
    if (girl.level() % 5 == 0)
    {
        ...
        while (addedtrait > 0)
        {
            ...
            if (!trait.empty() && girl.gain_trait(trait.c_str()))
            {
                ...
                girl.AddMessage(ss.str(), EBaseImage::PROFILE, EVENT_GOODNEWS);
            }
            addedtrait--;
        }
    }
...
void cGirls::EndDayGirls(IBuilding& brothel, sGirl& girl)
{
    ...
    if (goodnews.str().length() > 2)    girl.AddMessage(goodnews.str(), EBaseImage::PROFILE, EVENT_GOODNEWS);
    ...
    if (girl.has_active_trait(drug_trait))
    {
        ...
        if (!temp)    // withdrawals for a week
        {
            if (girl.m_Withdrawals >= withdrawal_duration)
            {
                ...
                girl.AddMessage(goodnews.str(), EBaseImage::PROFILE, EVENT_GOODNEWS);
            }
            else
            {
        ...
        else if (auto item = girl.has_item("Stop Smoking Patch"))
        {
            ...
            if(HandleDrug(girl, "Smoker", "Nicotine",
                ...
                if (girl.is_dead()) {
                    ...
                    girl.AddMessage(cancer.str(), EBaseImage::PROFILE, EVENT_WARNING);
                    return;
                }
            }
        }
...
bool cGirls::PossiblyGainNewTrait(sGirl& girl, string Trait, int Threshold, int ActionType, string Message, bool Day0Night1, EventType eventtype)
{
    if (girl.m_Enjoyment[ActionType] > Threshold)
    {
        ...
        {
            girl.AddMessage(Message, EBaseImage::PROFILE, eventtype);
            return true;
        }
    }
    return false;
...
bool cGirls::PossiblyLoseExistingTrait(sGirl& girl, string Trait, int Threshold, int ActionType, string Message, bool Day0Night1)
{
    if (girl.m_Enjoyment[ActionType] > Threshold && girl.has_active_trait(Trait.c_str()))
    {
        ...
        {
            girl.AddMessage(Message, EBaseImage::PROFILE, EVENT_GOODNEWS);
            return true;
        }
    }
    return false;
...
std::string AdjustTraitGroup(sGirl& girl, int adjustment, std::initializer_list<const char*> traits, bool event,
    ...
    if (event)    girl.AddMessage(ss.str(), EBaseImage::PROFILE, EVENT_GOODNEWS);
    ...
    if (girl.has_active_trait("Pessimist"))
    {
        ...
        if (girl.happiness() <= 0 && g_Dice.percent(50))
        {
            ...
            else
            {
                ...
                girl.AddMessage(msg, EBaseImage::DEATH, EVENT_DANGER);
...
void cGirls::GirlFucks(sGirl* girl, bool Day0Night1, sCustomer* customer, bool group, string& message, SKILLS &SexType, bool first)
{
    ...
    {
        girl->AddMessage("${name} gave the customer AIDS! They are not happy about this.", EBaseImage::PROFILE, EVENT_DANGER);
        customer->happiness(-100);
        enjoy -= 3;
    }
    else if (customer->has_active_trait("AIDS") && girl->gain_trait("AIDS", STDchance))
    {
        girl->AddMessage("${name} has caught the disease AIDS! She will likely die, but a rare cure can sometimes be found in the shop.", EBaseImage::PROFILE, EVENT_DANGER);
        girl->happiness(-50);
        enjoy -= 30;
    }
    if (girl->has_active_trait("Chlamydia") && customer->gain_trait("Chlamydia", STDchance))
    {
        girl->AddMessage("${name} gave the customer Chlamydia! They are not happy about this.", EBaseImage::PROFILE, EVENT_DANGER);
        customer->happiness(-40);
        enjoy -= 3;
    }
    else if (customer->has_active_trait("Chlamydia") && girl->gain_trait("Chlamydia", STDchance))
    {
        girl->AddMessage("${name} has caught the disease Chlamydia! A cure can sometimes be found in the shop.", EBaseImage::PROFILE, EVENT_DANGER);
        girl->happiness(-30);
        enjoy -= 30;
    }
    ...
    {
        girl->AddMessage("${name} gave the customer Syphilis! They are not happy about this.", EBaseImage::PROFILE, EVENT_DANGER);
        customer->happiness(-50);
        enjoy -= 3;
    }
    else if (customer->has_active_trait("Syphilis") && girl->gain_trait("Syphilis", STDchance))
    {
        girl->AddMessage("${name} has caught the disease Syphilis! This can be deadly, but a cure can sometimes be found in the shop.", EBaseImage::PROFILE, EVENT_DANGER);
        girl->happiness(-30);
        enjoy -= 30;
    }
    ...
    {
        girl->AddMessage("${name} gave the customer Herpes! They are not happy about this.", EBaseImage::PROFILE, EVENT_DANGER);
        customer->happiness(-30);
        enjoy -= 3;
    }
    else if (customer->has_active_trait("Herpes") && girl->gain_trait("Herpes", STDchance))
    {
        girl->AddMessage("${name} has caught the disease Herpes! A cure can sometimes be found in the shop.", EBaseImage::PROFILE, EVENT_DANGER);
        girl->happiness(-30);
        enjoy -= 30;
    }
    ...
    */
    ...
    if(!handler)
        handler = [&](std::string message){
            girl.AddMessage(std::move(message), EBaseImage::PROFILE, EVENT_WARNING);
    };
