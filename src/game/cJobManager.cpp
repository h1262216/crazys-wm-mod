...
bool cJobManager::work_related_violence(sGirl& girl, bool Day0Night1, bool streets)
{
    ...
    if ((girl.has_active_trait("Yandere") || girl.has_active_trait("Tsundere") || girl.has_active_trait("Aggressive")) && g_Dice.percent(30))
    {
        switch (g_Dice % 5)
            {
            case 0:
                girl.AddMessage(("She beat the customer silly."), EBaseImage::COMBAT, Day0Night1 ? EVENT_NIGHTSHIFT : EVENT_DAYSHIFT);
                break;
            case 1:
                girl.AddMessage(("The customer's face annoyed her, so she punched it until it went away."), EBaseImage::COMBAT, Day0Night1 ? EVENT_NIGHTSHIFT : EVENT_DAYSHIFT);
                break;
            case 2:
                girl.AddMessage(("The customer acted like he owned her - so she pwned him."), EBaseImage::COMBAT, Day0Night1 ? EVENT_NIGHTSHIFT : EVENT_DAYSHIFT);
                break;
            case 3:
                girl.AddMessage(("The customer's attitude was bad. She corrected it."), EBaseImage::COMBAT, Day0Night1 ? EVENT_NIGHTSHIFT : EVENT_DAYSHIFT);
                break;
            case 4:
                girl.AddMessage(("He tried to insert a bottle into her, so she 'gave it' to him instead."), EBaseImage::COMBAT, Day0Night1 ? EVENT_NIGHTSHIFT : EVENT_DAYSHIFT);
                break;
            default:
                girl.AddMessage(("Did som(E)thing violent."), EBaseImage::COMBAT, Day0Night1 ? EVENT_NIGHTSHIFT : EVENT_DAYSHIFT);
                break;
            }
        girl.fame(-1);
    }
...
bool cJobManager::security_stops_rape(sGirl& girl, sGang *enemy_gang, int day_night)
{
    ...
    if (result == ECombatResult::DEFEAT)  // Security guard wins
    {
        ...
        if (OrgNumMem == 1)
        {
            ...
            if (female)
            {
                ...
                if (!item.empty())
                {
                    ...
                    custgirl->AddMessage(CGmsg.str(), EBaseImage::DEATH, EVENT_WARNING);
        ...
        SecGuard->AddMessage(SGmsg.str(), EBaseImage::COMBAT, EVENT_WARNING);
        girl.AddMessage(Gmsg.str(), EBaseImage::COMBAT, EVENT_WARNING);
        return true;
    }
        ...
        SecGuard->AddMessage(ss.str(), EBaseImage::DEATH, EVENT_DANGER);
        return false;
    }
    else  // Loss
    {
...
bool cJobManager::gang_stops_rape(sGirl& girl, std::vector<sGang *> gangs_guarding, sGang *enemy_gang, int coverage, int day_night)
{
    ...
    if (result == EFightResult::DEFEAT) {
        ...
        girl.AddMessage(girl_s.str(), EBaseImage::DEATH, EVENT_WARNING);
        return false;
    }
    ...
    girl.AddMessage(girl_ss.str(), EBaseImage::PROFILE, EVENT_WARNING);
...
bool cJobManager::girl_fights_rape(sGirl& girl, sGang *enemy_gang, int day_night)
{
    ...
    if (res)  // She wins.  Yay!
    {
        ...
        girl.AddMessage(msg.str(), EBaseImage::COMBAT, EVENT_WARNING);
    }
...
void cJobManager::handle_simple_job(sGirl& girl, bool is_night)
{
    ...
    else
    {
        ...
        girl.AddMessage(ss.str(), EBaseImage::PROFILE, EVENT_SUMMARY);
    }
}
...
bool cJobManager::AddictBuysDrugs(std::string Addiction, std::string Drug, sGirl& girl, IBuilding * brothel, bool Day0Night1)
{
    ...
    if ((brothel->num_girls_on_job(JOB_MATRON, true) >= 1 || brothel->num_girls_on_job(JOB_MATRON, false) >= 1)
        && g_Dice.percent(70))
    {
        girl.AddMessage("Matron confiscates drugs", EBaseImage::PROFILE, EVENT_WARNING);
